...
const VOICE_LINES := {
	...
	"see_boss": preload("res://sounds/VL/thats one big bat.mp3"),
	"beat_the_game": preload("res://sounds/VL/gg thanks for playing.mp3"),
	
	"beat_wave20": preload("res://sounds/VL/10 left good going.mp3"),
	"beat_wave1": preload("res://sounds/VL/10 left good going.mp3"),
	...
	"250_coins_run": preload("res://sounds/VL/that was a good run.mp3"),
...
func play_vl(vl: String):
	if (vl in single_activation_vls and
		single_activation_vls[vl] == false):
		...
		if VOICE_LINES[vl] is Dictionary:
			var random_key = VOICE_LINES[vl].keys().pick_random()
			var random_vl = VOICE_LINES[vl][random_key]
			stream = random_vl
			self.play()
